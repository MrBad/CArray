...
/**
 * Grows/doubles the capacity of the array
 */
...
{
    void *p;
    p = bsearch(element, array->data, array->length, array->elementSize, func);
    if (!p){
        return -1;
    }
    return ((char *)p - (char *)array->data) / array->elementSize;
}
...
int cmpPersons(const void *a, const void *b)
{
    const Person *p1 = a;
    const Person *p2 = b;
...
bool arrayTest()
{
    ...
    p = arrayNewElement(arr);
    strncpy(p->name, "Popescu Ion", sizeof(p->name));
    p->age = 32;

    arraySort(arr, cmpPersons);
 
    Person person = {"Popescu Ion", 44};
    printf("Index of: %s is %d\n", person.name, arrayIndexOf(arr, &person, cmpPersons));
    int idx = arrayIndexOf(arr, &person, cmpPersons);
    assert(idx == 2);
